...
impl Tlb {
    ...
    pub fn lookup(&mut self, vpn: u64, asid: u16) -> Option<u64> {
        ...
        for i in &mut self.entries{
            if i.valid & (i.vpn == vpn) & (i.asid == asid){
                self.stats.hits += 1;
                return Some(i.ppn);
            }
        }
...
impl Mmu {
    ...
    pub fn translate(&mut self, vpn: u64) -> Option<u64> {
        // TODO: 实现 TLB + 页表的二级查找
        let a = self.tlb.lookup(vpn, self.current_asid);
        match a {
            Some(e) => {return a;},
            None => {},
        }
        for (m,n) in &mut self.page_table{
            if (n.vpn == vpn) & (self.current_asid == *m){
                self.tlb.insert(vpn, n.ppn, self.current_asid, n.flags);
                return Some(n.ppn);
            }
        }
        return None;
