...
impl SingleLevelPageTable {
    ...
    ///
    /// 步骤：
    /// 1. 从虚拟地址中提取 VPN（高 20 位）和 ovpnffset（低 12 位）
    ...
    pub fn translate(&self, va: u32, is_write: bool) -> TranslateResult {
        ...
        let vpn = (va >> PAGE_OFFSET_BITS) as usize;
        let offset = va & ((1 << PAGE_OFFSET_BITS) - 1);
        let a = self.lookup(vpn);
        let d = match a {
            Some(c) => c,
            None => {return PageFault;},
        };
        if PTE_VALID  & d.flags == 0{
            return PageFault;
        }
        if (is_write == true) & (PTE_WRITE & d.flags == 0){
            return PermissionDenied;
        }
        Ok(d.ppn * PAGE_SIZE as u32 + offset)
    }
}
