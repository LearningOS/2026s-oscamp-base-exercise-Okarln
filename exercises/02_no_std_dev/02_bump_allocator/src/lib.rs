...
unsafe impl GlobalAlloc for BumpAllocator {
    unsafe fn alloc(&self, layout: Layout) -> *mut u8 {
        ...
        let mut allo = self.next.load(Ordering::SeqCst);
        loop{
            let aligned_start = (allo + layout.align() - 1) & !(layout.align() - 1);
            let aligned_end = aligned_start + layout.size();
            if aligned_end > self.heap_end{
                return null_mut();
            }
            let m = match  self.next.compare_exchange(allo, aligned_end, Ordering::SeqCst, Ordering::SeqCst){
