...
impl<T> RwLock<T> {
    ...
    pub fn read(&self) -> RwLockReadGuard<'_, T> {
        ...
        loop {
            ...
            if (WRITER_HOLDING & s) != 0|| (WRITER_WAITING & s) != 0{
                core::hint::spin_loop();
                continue;
            }
            ...
            let a = self.state.compare_exchange(s, s+1, Ordering::AcqRel, Ordering::Acquire);
            match a {
                Ok(_) => {
    ...
    pub fn write(&self) -> RwLockWriteGuard<'_, T> {
        ...
        loop{
            ...
            if (WRITER_HOLDING & s) != 0|| (READER_MASK & s) != 0{
                core::hint::spin_loop();
                continue;
            }
            let a = self.state.compare_exchange(WRITER_WAITING, WRITER_HOLDING, Ordering::AcqRel, Ordering::Acquire);
            match a {
                Ok(_) => {
