...
pub fn run_command(program: &str, args: &[&str]) -> String {
    ...
    let a = Command::new(program).args(args).stdout(Stdio::piped()).output();
    let b = String::from_utf8(a.unwrap().stdout).unwrap();
    b
...
pub fn pipe_through_cat(input: &str) -> String {
    ...
    child.stdout.take().unwrap().read_to_string(a).unwrap();
    a.to_string()
}
...
pub fn get_exit_code(command: &str) -> i32 {
    ...
    let ec = exit_status.code().unwrap();
    ec
}
...
pub fn run_command_with_result(program: &str, args: &[&str]) -> io::Result<String> {
    ...
    let res = Command::new(program).args(args).stdout(Stdio::piped()).output();
    match res {
        Ok(e) => String::from_utf8(e.stdout).map_err(|err|std::io::Error::new(std::io::ErrorKind::InvalidData
  , err)),
        Err(e) => Err(e),
    }
}
...
pub fn pipe_through_grep(pattern: &str, input: &str) -> String {
    ...
    let mut a = Command::new("grep").arg(pattern).stdin(Stdio::piped()).stdout(Stdio::piped()).spawn().unwrap();
    a.stdin.as_mut().unwrap().write(input.as_bytes()).unwrap();
    drop(a.stdin.take());
    let mut b = String::new();
    a.stdout.unwrap().read_to_string(&mut b).unwrap();
    b
}
